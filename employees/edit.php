...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $update = $pdo->prepare('UPDATE employees SET employee_no=?, first_name=?, last_name=?, email=?, phone=?, hire_date=?, department_id=?, position_id=?, status_id=?, address=? WHERE id=?');
        $update->execute([$employee['employee_no'], $employee['first_name'], $employee['last_name'], $employee['email'], $employee['phone'], $employee['hire_date'], $employee['department_id'], $employee['position_id'], $employee['status_id'], $employee['address'], $id]);
...
?>
<main><div class="container-fluid px-4"><h1 class="mt-4">Edit Employee</h1><?php require __DIR__ . '/form.php'; ?></div></main>
